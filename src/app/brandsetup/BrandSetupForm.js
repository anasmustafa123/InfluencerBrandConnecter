...
import { completeBrandSetup } from "@/lib/profils/brand_profile";

export default function BrandSetupForm({ brandData, userId, userRole, brand_id }) {
  const [step, setStep] = useState(1);
  const router = useRouter();
  
  // Initialize form data with existing brand data
  const [formData, setFormData] = useState({
    brandName: brandData?.name || "",
    brandBio: brandData?.tag || brandData?.header_info || "",
    region: brandData?.location || "",
    website: brandData?.website_url || "",
    product1: "",
    product2: "",
    product3: "",
    contactEmail: brandData?.contact_email || "",
    contactPhone: brandData?.contact_phone || ""
  });
  ...
  const handleSetupComplete = async () => {
    try {
      const result = await completeBrandSetup(brand_id, formData);
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <textarea 
                ...
                onChange={(e) => handleInputChange('brandBio', e.target.value)}
                rows="3"
              />
              <input 
                className="w-full p-2 border rounded" 
          ...
          <div>
            ...
            <div className="space-y-4">
              <input 
                ...
                onChange={(e) => handleInputChange('product1', e.target.value)}
              />
              <input 
                className="w-full p-2 border rounded" 
                placeholder="Product 2"
                value={formData.product2}
                onChange={(e) => handleInputChange('product2', e.target.value)}
              />
              <input 
                className="w-full p-2 border rounded" 
                placeholder="Product 3"
                value={formData.product3}
                onChange={(e) => handleInputChange('product3', e.target.value)}
              />
              <input 
                className="w-full p-2 border rounded" 
